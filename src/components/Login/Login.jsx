...
const Login = () => {
  ...
  const handleLoginSubmit = async (e) => {
    ...
    const { sucesso, usuarioDecodificado, erro } = await handleSubmit(
      login,
      senha
    );
    ...
    if (sucesso) {
      if (sucesso && usuarioDecodificado.role === 'USER' ) 
        navigate("/Home");
      if (sucesso && usuarioDecodificado.role === 'ADMINISTRATOR')
        navigate("/HomeAdmin");
      if (sucesso && usuarioDecodificado.role === "MODERATOR")
        navigate("/HomeMod");
    } else {
      setError(erro);
      setValidaPop(true);
      ...
      <div className="min-h-screen  bg-white flex">
        <div className="flex-1 p-4">
          <div className="mx-auto w-full max-w-sm">
            <h1 className="mb-12 text-3xl h-20 w-auto  text-cinza font-extralight ">
              <img className="h-auto w-7/12 items-center mx-auto " src="Bon.png"></img>
            </h1>
          </div>
          <div className="mt-2">
